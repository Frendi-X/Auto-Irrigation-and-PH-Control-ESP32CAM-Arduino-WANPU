...
#define Buzzer 4
#define sensorPIR 5
#define SP_tanahPin A0       // A1 -> Pin Potensio Set Nilai tanah
#define SP_PHPin A1          // A2 -> Pin Potensio Set Nilai PH
#define SoilMoisture_Pin A2  // A3 -> Pin Sensor Soil Moisture (Kelembaban Tanah)
#define adaGerakan 1
...
void handlePIR() {
  ...
  if (beepStep == 1 && millis() - beepTimer >= 0) {
    ...
    digitalWrite(Buzzer, HIGH);  // Beep ON (kedua)
    digitalWrite(ledPin, HIGH);
    beepTimer = millis();
    beepStep = 4;
  }
...
void loop() {
  ...
  // Simulasi pH naik-turun via potensiometer (A1)
  PH = mapFloat(analogRead(SP_PHPin), 0, 1023, 0, 14);
  delay(10);
